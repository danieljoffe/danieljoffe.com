...
- **Show real code.** The prose explains the _why_, the code shows the _what_. Minimal inline comments.
- **Close with a principle.** Active voice, first-person plural. "We find the narrowest neutral state" not "It's to find the narrowest state." Sound like a colleague, not a textbook.

### Punctuation

- **Prefer periods and colons over em dashes for clause breaks.** Em dashes create choppiness when overused. Use them sparingly for true asides.
  - YES: "The error itself is harmless. React recovers by re-rendering."
  - YES: "Modern browsers prefer the SVG when both are available: it scales perfectly."
...
1. **Em dash audit.** Replace most with periods or colons.
2. **Consequence check.** Every "X happened" should have a "because Y" or "which meant Z."
3. **Takeaway voice.** Read it aloud. Does it sound like a colleague talking, or a textbook?
4. **Full artifacts.** If the post is about a script or config, is the complete version included?
