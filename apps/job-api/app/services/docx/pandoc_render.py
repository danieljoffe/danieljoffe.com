...
The pandoc binary must be available on PATH. The job-api Dockerfile
installs it via apt; local dev needs `brew install pandoc` or equivalent.
`PandocNotInstalled` is raised loud if the binary is missing — there's
...
def md_to_docx(markdown: str) -> bytes:
    ...
    try:
        ...
        raise PandocNotInstalled(str(exc)) from exc
