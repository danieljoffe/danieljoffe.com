...
export const setGradientTheme = (target: HTMLElement): void => {
  ...
  Object.entries(randomTheme).forEach(([key, value]: [string, string]) => {
    // Register CSS property type once to enable smooth CSS transitions
    if (
      ...
      if (!win.__registeredCSSProps.has(key)) {
        try {
          (
          ...
          win.__registeredCSSProps.add(key);
        } catch {
    ...
    // Set the value on the scoped target element (not documentElement)
