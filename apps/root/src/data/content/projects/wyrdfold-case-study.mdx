...
**Purpose:** WyrdFold started as a quick script to scrape jobs out of a few boards, and I added one feature at a time until it outgrew the monorepo it was living in and moved into its own repo with full CI. This is the project where I stopped owning just the frontend and started owning the whole stack: a Next.js web app, a FastAPI matching service, a Postgres schema, and an LLM pipeline running in production, plus all the cost and quality guardrails you need before you trust a model in front of real users.
...
Grading every posting with a full LLM call is the obvious approach, and it's the wrong one. Most postings are off-target on the title alone, and paying for a deep reasoning pass on a job that was never a fit just burns budget for no signal. So matching runs in two phases. A cheap title-triage pass scores thousands of jobs per target and decides which ones are even worth a closer look; only the survivors reach the expensive, in-depth fit grade.
