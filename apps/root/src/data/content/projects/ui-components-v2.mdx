export const metadata = {
  ...
  type: 'project',
  cover: {
    alt: 'An abstract composition of colorful geometric blocks',
    src: '/images/covers/ui-components-v2.webp',
  },
};

## Expanding the Design System: UI Components Part 2

Building on the [foundational components](/projects/ui-components-v1), this second round added the form controls, feedback mechanisms, and layout primitives, so the library now covers most of the common UI patterns I reach for.

## Business Impact

- Expanded the component library with form controls (Checkbox, Select, Switch) enabling rapid form development
- Maintained design consistency across all interactive elements
- Reduced time-to-implementation for new form-based features
...
<InteractiveDemo
  ...
  height={150}
  controls={[
    { label: 'Info', args: { variant: 'info' } },
    { label: 'Success', args: { variant: 'success' } },
    { label: 'Warning', args: { variant: 'warning' } },
    { label: 'Error', args: { variant: 'error' } },
  ]}
/>
...
The shared-ui library is framework-agnostic by design; it depends only on React and Tailwind CSS, with no `next/link`, `next/image`, `useRouter`, or any other Next.js API in sight. That separation is deliberate, and it gives me a clear two-layer architecture:
...
The app-level `Button` component (`components/Button.tsx`) wraps the shared-ui `Button` to add Next.js `Link` integration: when it renders as a link, it uses `next/link` for client-side navigation with prefetching, and the shared-ui `Button` underneath knows nothing about routing at all.

Kit components like `PostCard` work the same way, composing shared-ui primitives (`Badge`, `Heading`, `Text`) with the Next.js-specific concerns like image optimization and route-aware links.

### React 19 migration

Somewhere between V1 and V2, React 19 deprecated `forwardRef`, so the shared-ui library now accepts `ref` as a regular prop in the component interface:
...
That cleared out every `forwardRef` wrapper, along with the `Client*` wrapper pattern I'd previously needed just to keep refs working at `'use client'` boundaries.

### Why this matters

Because of this architecture, the shared-ui library can be consumed by _any_ React project, not just this Next.js app. A Vite app, a Remix app, or a React Native Web project could all pull in the same components, and the framework-specific adaptation stays in the consuming app where it belongs.

## What I Learned

**Composition beats configuration.** The Card started life as one monolithic prop-based component, and breaking it into CardHeader, CardTitle, and CardContent made it far more flexible.

**ARIA roles matter.** The Tabs component needed careful attention to the `tablist`, `tab`, and `tabpanel` roles, and getting keyboard navigation right took me a few iterations.

**Test interactions, not implementation.** Storybook's play functions let me test the real user flows (clicking, typing, tabbing) instead of poking at internal state.
...
_A design system is never finished; it keeps evolving alongside the product it serves, and that's the point._
