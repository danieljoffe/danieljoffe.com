...
initial pathname. The result: the server marks "Home" as active, the client
marks "Projects" as active, and React throws a hydration error.
...
first paint, and it introduces a layout shift when the nav pops in after
hydration.
...
suppress the warning. It's to find the narrowest possible neutral state
that both environments agree on, then upgrade to the real value after
hydration. `useSyncExternalStore` makes that a three-line hook.
