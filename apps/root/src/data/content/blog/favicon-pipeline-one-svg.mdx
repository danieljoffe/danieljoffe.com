export const metadata = {
  ...
  cover: {
    ...
    src: '/images/covers/favicon-pipeline-one-svg.webp',
  },
};
...
an SVG to some "free" web tool, downloading a zip, extracting it, and hand-copying
files into `public/`. The output was never consistent either: different tools
give you different compression, different background handling, and different
size matrices, and none of it lives in version control.

What I wanted was a deterministic local pipeline: one SVG in, all the meta images
out.

## What Browsers Actually Need

The favicon landscape has consolidated a lot, so here's what modern browsers and
...
That's 11 files from one source SVG, and the `favicon.ico` is really three
PNGs bundled into one file (16, 32, and 48px) so the browser can pick
the best size for the context.

## The Script

The pipeline leans on two tools: `rsvg-convert` for SVG-to-PNG rasterization
(from the `librsvg` package) and ImageMagick's `magick` for combining the
PNGs into a multi-resolution `.ico`. Both come from Homebrew.
...
The `.ico` combines three sizes into one file, and that's what makes it
...
Finally, I copy the source SVG over as `favicon.svg` for browsers that
support it. Modern browsers prefer the SVG when both are available, since it
scales perfectly at any resolution and can do dark mode via
`prefers-color-scheme` media queries baked into the SVG itself.

## Making It Reusable

The script takes two arguments: the source SVG and an optional output
directory, which I defaulted to `apps/root/public/`:
...
after a logo change is a one-command job. The skill checks that
`rsvg-convert` and `magick` are installed, runs the script, and reports
back which files it generated.
...
filenames. The script lives in version control next to the source SVG,
so the pipeline is reproducible and the output is auditable. When the logo
changes, and it will, I run one command on my own machine instead of going back to some website.
