...
Not every toast deserves the same urgency, and the ARIA spec gives me two
options:
...
The distinction matters: an "email sent" confirmation can wait, but a "payment
failed" error shouldn't.
...
Auto-dismiss solves the sighted user's problem, since toasts go away on their
own, but a 4-second timer is hostile if the user is still reading. So the fix
is to pause the timer while the user is engaged.
...
Small detail, big payoff. A screen reader user now hears "Dismiss
