...
Two details matter. First, the job installs both pnpm _and_ uv: pnpm to get the Nx CLI, uv to execute the actual Python work dispatched by Nx. Second, `pnpm nx run-many -t lint test mypy -p job-api` runs the three Python targets in parallel; `--nxBail` kills the run on the first failure.
