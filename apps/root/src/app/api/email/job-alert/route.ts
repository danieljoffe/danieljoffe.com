...
export async function POST(request: NextRequest) {
  ...
  const unsubscribeUrl = buildProfileUnsubscribeUrl(
    payload.profileId,
    'job_alerts',
    siteUrl
  );
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      // RFC 2369 + RFC 8058: Gmail/Yahoo render a native unsubscribe button
      // and POST to the URL on click, no user-visible page round-trip.
      headers: {
