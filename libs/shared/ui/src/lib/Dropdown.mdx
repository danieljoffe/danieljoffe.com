...
<table className='doc-table'>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Renders the item as an anchor pointing here instead of a button.</td>
    </tr>
    <tr>
      <td>
        <code>external</code>
      ...
      <td>
        Opens <code>href</code> in a new tab with a trailing external-link icon.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        Pending state: spinner in the icon slot, <code>aria-busy</code>, and
        non-actionable until it resolves.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        Custom item body replacing the icon/label rendering; <code>label</code>{' '}
    ...
    <tr>
      ...
      <td>
        Set <code>false</code> to keep the menu open after activation (async
        pickers). Defaults to <code>true</code>.
      </td>
    </tr>
  </tbody>
</table>
...
<p className='doc-p'>
  For items that trigger an async action (e.g. "add to list"), set{' '}
  <code>closeOnClick: false</code> so the menu stays open, flip{' '}
  <code>loading: true</code> while the action runs, and mark completed items{' '}
  <code>disabled</code>. For richer rows (title + subtitle), pass{' '}
  <code>content</code>; the required <code>label</code> stays the accessible
  name. If the picker needs its own loading/error/empty states outside the
...
<Dropdown
  ...
  items={targets.map(target => ({
    ...
    disabled: addedIds.includes(target.id),
    closeOnClick: false,
    onClick: () => addToTarget(target.id),
  }))}
/>
```
